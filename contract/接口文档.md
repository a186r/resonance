...
这里返回该轮次funder的address(funderAddress)和组建期投入的token数量(tokenAmount)，需要根据tokenAmount对funder进行排序，排序结果会用于裂变奖励活动中

- 接口名称：`getCurrentStepFunders`
